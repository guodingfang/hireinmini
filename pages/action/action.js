...
var getCalendarList = function (companyid, date_query, that, choosedDay, isexpand) {
    wx.request({
        url: Config.baseUrl + "Lease/actionDayTask",
        data: {
            companyid: companyid,
...
/*根据日期获取订单列表*/
var getOrderlistByDate = function (companyid, employeeid, date_query, that) {
    wx.request({
        url: Config.baseUrl + "Lease/action_bgo",
        data: {
            companyid: companyid,
            date_query: date_query,
...
/*取消操作*/
var cancelOrder = function (orderid, index, that) {
    wx.request({
        url: Config.baseUrl + "Lease/order_cancel",
        data: {
            orderid: orderid,
        },
        header: {
...
const conf = {
    ...
    onLoad() {
        ...
        /*获取活动订单*/
        this.getOrderlistByDate(this.data.companyid, this.data.employeeid, cur_year + "-" + cur_month + "-" + cur_date);
    },
    onShow() {
        // 获取信息
        getInfo(this);
    ...
    tapDayItem(e) {
        ...
        /*获取活动订单*/
        this.getOrderlistByDate(this.data.companyid, this.data.employeeid, param_fullday);
    },
    /*点击年月，弹出年月的选择页面*/
    chooseYearAndMonth() {
    ...
    tapPickerBtn(e) {
        ...
        if (type === 'confirm') {
            ...
            /*获取派单订单*/
            this.getOrderlistByDate(this.data.companyid, this.data.employeeid, choose_year + "-" + choose_month + "-" + choosed_day);
        }
    ...
    /*根据日期获取订单信息*/
    getOrderlistByDate(companyid, employeeid, date_query) {
        getOrderlistByDate(companyid, employeeid, date_query, this);
    },
    /**
    * 跳转到主页
    */
    ...
    getSpecifiedCalendar:function(type){
        ...
        if(type == "next"){
            ...
            /*获取派单订单*/
            this.getOrderlistByDate(this.data.companyid, this.data.employeeid, newYear+"-"+newMonth+"-"+cur_day);
            this.setData({
                cur_year: newYear,
                cur_month: newMonth
            });
     ...
     */
    skip: function (e) {
        var orderid = config.getDataSet(e, 'order');
        config.permission('client_order_info/client_order_info', function () {
            wx.navigateTo({
                url: '../client_order_info/client_order_info?orderid=' + orderid + '&leaseorderid=' + orderid,
            })
        });
    }
};
