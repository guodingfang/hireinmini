...
/*获取日历循环列表数据*/
var getCalendarList = function(companyid, date_query, that, choosedDay, isexpand) {
    wx.request({
        url: Config.baseUrl + "Lease/arrangeDayTask",
        data: {
            companyid: companyid,
...
/*根据日期获取订单列表*/
var getOrderlistByDate = function(companyid, date_query, that) {
    wx.request({
        url: Config.baseUrl + "Lease/arrangelist",
        data: {
            companyid: companyid,
            date_query: date_query
        },
...
/*取消操作*/
var cancelOrder = function(orderid, stepid, companyid, orderindex, arrangeindex, that) {
    wx.request({
        url: Config.baseUrl + "Lease/orderArrange_cancel",
        data: {
            orderid: orderid,
            stepid: stepid,
            companyid: companyid
            ...
            if (res.data.result) {
                wx.showToast({
                    ...
                    success: function(e) {
                        ...
                        orderlist[orderindex]['arrange'].splice(arrangeindex, 1);
                        that.setData({
                            orderlist: orderlist
                        })
                    }
...
const conf = {
    ...
    onLoad: function(options) {
        ...
        this.calculateDays(cur_year, cur_month, cur_year + "-" + cur_month + "-" + cur_date, 1); //日历循环

        /*获取派单订单*/
        this.getOrderlistByDate(this.data.companyid, cur_year + "-" + cur_month + "-" + cur_date);
    },
    onShow() {
        // 获取信息
        getInfo(this);
    ...
    tapDayItem(e) {
        ...
        /*获取派单订单*/
        this.getOrderlistByDate(this.data.companyid, param_fullday);
    },
    /*点击年月，弹出年月的选择页面*/
    chooseYearAndMonth() {
    ...
    tapPickerBtn(e) {
        ...
        if (type === 'confirm') {
            ...
            /*获取派单订单*/
            this.getOrderlistByDate(this.data.companyid, choose_year + "-" + choose_month + "-" + choosed_day);
        }
    ...
    calculateDays(year, month, choosedDay, isexpand) {
        ...
        getCalendarList(this.data.companyid, firstCalendarDay, this, choosedParamsDay, isexpand);
    },
    /*获取某个值在某个数组中是否存在*/
    inArray(arr, obj) {
        var i = arr.length;
    ...
    cancelArrange(e) {
        ...
        var stepid = config.getDataSet(e, "stepid");
        var orderindex = config.getDataSet(e, "orderindex");
        var arrangeindex = config.getDataSet(e, "arrangeindex");
        ...
        wx.showModal({
            ...
            success: function(res) {
                if (res.confirm) {
                    /*删除请求后台*/
                    cancelOrder(orderid, stepid, companyid, orderindex, arrangeindex, that);
                    /*删除请求后台*/
                } else {
                    return;
                }
    ...
    /*根据日期获取订单信息*/
    getOrderlistByDate(companyid, date_query) {
        getOrderlistByDate(companyid, date_query, this);
    },
    // 触摸开始事件
    ...
    getSpecifiedCalendar: function(type) {
        ...
        if (type == "next") {
            ...
            /*获取派单订单*/
            this.getOrderlistByDate(this.data.companyid, newYear + "-" + newMonth + "-" + cur_day);
